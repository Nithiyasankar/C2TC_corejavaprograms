package com.tnsif.dayfifteen.generics;

import java.util.Arrays;
import java.util.List;

public class LowerBoundedWildCardDemo {

    public static void main(String[] args) {
        // List of Integers
        List<Integer> intList = Arrays.asList(1, 2, 3);
        System.out.println("Display Integer values:");
        LowerBoundedWildCard.displayNumbers(intList);

        // List of Numbers
        List<Number> numberList = Arrays.asList(1.0, 2.0, 3.0, 10, 4, 30);
        System.out.println("Display Number values:");
        LowerBoundedWildCard.displayNumbers(numberList);

        // List of Doubles (Double is NOT a superclass of Integer → so won't compile if used directly)
        // Correct way: Use Number instead of Double for lower bounded wildcard
        List<Number> doubleList = Arrays.asList(1.0, 2.0, 3.0, 0.9);
        System.out.println("Display Double values:");
        LowerBoundedWildCard.displayNumbers(doubleList);
    }
}